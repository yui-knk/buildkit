...
func TestShellParser4EnvVars(t *testing.T) {
	...
	envsKvpos := stringsToKeyValuePairOptionals(envs)
	for scanner.Scan() {
		line := scanner.Text()
		lineCount++
		...
		if ((platform == "W" || platform == "A") && runtime.GOOS == "windows") ||
			...
			newWord, err = shlex.ProcessWordWithKvpos(source, envsKvpos)
			if expected == "error" {
				assert.Check(t, is.ErrorContains(err, ""), "input: %q, result: %q", source, newWord)
			} else {
				assert.Check(t, err, "at line %d of %s", lineCount, fn)
...
func TestGetEnv(t *testing.T) {
	...
	sw.envs = stringsToKeyValuePairOptionals([]string{})
	if sw.getEnv("foo") != "" {
		t.Fatal("2 - 'foo' should map to ''")
	}

	sw.envs = stringsToKeyValuePairOptionals([]string{"foo"})
	if sw.getEnv("foo") != "" {
	...
	sw.envs = stringsToKeyValuePairOptionals([]string{"foo=bar"})
	if sw.getEnv("foo") != "bar" {
		t.Fatal("5 - 'foo' should map to 'bar'")
	}

	sw.envs = stringsToKeyValuePairOptionals([]string{"foo=bar", "car=hat"})
	if sw.getEnv("foo") != "bar" {
	...
	sw.envs = stringsToKeyValuePairOptionals([]string{"foo=bar", "car=hat", "car=bike"})
	if sw.getEnv("car") != "hat" {
